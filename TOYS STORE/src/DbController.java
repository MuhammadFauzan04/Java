...
public class DbController extends MyConfig {
    ...
    public static List<Produk> getDatabase() {
        ...
        try {
            ...
            query = "SELECT id, nama, harga, jumlah FROM tb_souvenir";
    ...
    public static Produk getProdukByNama(String nama) {
        Produk produk = null;
        connection();
        query = "SELECT * FROM tb_souvenir WHERE NAMA=?";
        try {
            preparedStatement = connection.prepareStatement(query);
    ...
    public static boolean insertData(String nama, long harga, int jumlah) {
        connection();
        query = "INSERT INTO tb_souvenir (NAMA, HARGA, jumlah) VALUES (?, ?, ?)";
        try {
            preparedStatement = connection.prepareStatement(query);
            preparedStatement.setString(1, nama);
    ...
    public static void updateNama(int id, String nama) {
        connection();
        query = "UPDATE tb_souvenir SET NAMA=? WHERE ID=?";
        try {
            preparedStatement = connection.prepareStatement(query);
            preparedStatement.setString(1, nama);
    ...
    public static void updateHarga(int id, long harga) {
        connection();
        query = "UPDATE tb_souvenir SET HARGA=? WHERE ID=?";
        try {
            preparedStatement = connection.prepareStatement(query);
    ...
    public static void updateJumlah(int id, int jumlah) {
        connection();
        query = "UPDATE tb_souvenir SET jumlah=? WHERE ID=?";
        try {
            preparedStatement = connection.prepareStatement(query);
    ...
    public static boolean deleteData(int id) {
        connection();
        query = "DELETE FROM tb_souvenir WHERE ID=?";
        try {
            preparedStatement = connection.prepareStatement(query);
